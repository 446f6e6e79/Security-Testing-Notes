...
Before discussing the Request Forgery vulnerability, it's important to understand how \textbf{HTTP} and \textbf{cookies} work, as they are the basis of this type of attack.
\subsection{HTTP and Cookies introduction}
HTTP is a protocol that only allows communication using \textbf{request} and \textbf{responses}. It's considered \textbf{stateless},
 meaning that natively, it doesn't allow to preserve the state of a user.
\\Typically, the \textbf{state} is usually maintained via the use of \textbf{cookies}, 
sent by the server to the client's browser. They typically contain information to identify the client, but are also used for:
\begin{itemize}
...
\textbf{Session Hijacking} is an attack in which an \textbf{attacker} takes over a \textbf{valid user session}, gaining unauthorized access to the web application.
\paragraph{Scenario:}
\begin{enumerate}
    \item An authorized user \textbf{logs} into a website;
    \item The web server generates a \textbf{session ID} and sends it to the client as a \textbf{cookie};
    \item While the user is connected, the attacker manages to \textbf{steal} the \textbf{session ID};
    \item The \textbf{attacker} uses the stolen session ID to \textbf{impersonate} the \textbf{authorized user}.
\end{enumerate}
\paragraph{How?}
...
\textbf{Cross-Site Request Forgery}(CSRF) is an attack in which an attacker tricks a victim’s browser 
into \textbf{sending unauthorized requests} to a \textbf{trusted web application} in which the victim is already authenticated. 
