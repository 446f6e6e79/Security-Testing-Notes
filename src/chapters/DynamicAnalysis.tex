...
\subsection{Fuzzing}
Fuzzing is a highly effective and automated security testing technique.
It involves generating random or semi-random inputs to a program and check for crashes, memory leaks, or unexpected behavior.
...
\end{itemize}
We are not testing functionality, so we don't need to check for expected outputs. The goal is to simply observe the 
program's behavior when subjected to these inputs.
...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
\subsubsection*{Application of fuzzing}
Fuzzing can be applied to different types of software:
\begin{itemize}
    \item \textbf{Desktop applications}: fuzzing can be used to test desktop applications, focusing on their UI, command line interfaces, and file handling capabilities;
    \item \textbf{Web applications}: fuzzing can be used to test web applications, focusing on URLs, forms and requests;
    \item \textbf{Protocols}: the fuzzer can send generated packets to the tested application or eventually, act as a proxy between two communicating entities;
    \item \textbf{Files format}: the fuzzer can generate malformed or unexpected files to test the application's file parsing capabilities.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Cons}:
    \begin{itemize}
        \item Tend to find simple bugs, while more complex vulnerabilities may require more sophisticated analysis;
        \item May generate a large number of false positives, that are hard to debug if we don't have access to the source code;
