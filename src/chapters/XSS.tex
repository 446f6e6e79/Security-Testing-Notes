...
\subsubsection{Reflected (non-persistent)}
...
\subsubsection{Stored (persistent)}
...
A form of XSS in which the entire unsafe data flow occurs in the browser.
For example, the malicious payload could be included by the attacker in the URL of the page, and the page's JavaScript
directly takes that data from the URL and inserts it into the DOM without proper validation or sanitization.
In this case, the server does not reflect or store the malicious input; instead, the vulnerability arises from client-side
JavaScript code that processes untrusted data without proper validation or sanitization.
\subsubsection*{Vulnerable example (client-side):}
\begin{lstlisting}[language=HTML]
  </head>
  Main Dashboard for
  <script>
...
\begin{verbatim}
http://example.com/dashboard.html?context=<script>alert('XSS')</script>
\end{verbatim}
When a victim visits this URL, the JavaScript extracts the \texttt{context} parameter
and writes it directly into the page with the \texttt{document.write} function,
resulting in the execution of the injected script.

\subsubsection{Reflected vs DOM-based XSS}
On the surface, reflected and DOM-based XSS may appear similar, as both involve injecting malicious scripts that are executed in the
victim's browser, without it being stored on the server.
However, the key difference lies in where the vulnerability exists:
\begin{itemize}
  \item \textbf{Reflected XSS} vulnerabilities exist on the server side. The user input is sent to the server,
  which then reflects it back in the HTTP response without proper sanitization;
  \item \textbf{DOM-based XSS} vulnerabilities exist entirely on the client side. This time, the malicious input is
  processed directly by the browser's JavaScript, without any server-side involvement.
\end{itemize}  
